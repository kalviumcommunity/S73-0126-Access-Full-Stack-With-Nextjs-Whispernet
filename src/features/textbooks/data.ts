// features/textbooks/data.ts
// The offline textbook library.
//
// Chapters are plain markdown strings held in the bundle rather than fetched:
// every book is then part of the statically generated build, which is what
// makes the library readable on a device that has been offline for a week.

export interface Chapter {
  id: number;
  title: string;
  /** Markdown source rendered by `renderChapter`. */
  content: string;
  /** Estimated reading time in minutes. */
  minutes: number;
}

/** Colour role for a subject, mapped to concrete classes in `subjectTheme`. */
export type SubjectAccent = "green" | "blue" | "violet" | "amber" | "rose";
...
export interface Textbook {
  ...
  title: string;
  description: string;
  /** Grades this book is written for, used by the grade filter. */
  grades: number[];
  /** Publishing body, as it would appear on a printed cover. */
  publisher: string;
  edition: string;
...
export const textbooks: Textbook[] = [
  {
    ...
    grades: [6, 7, 8],
    publisher: "State Council of Educational Research",
